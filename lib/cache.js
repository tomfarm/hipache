...
function Cache(config, handlers) {
  ...
  var logHandler = config.logger

  this.config = config;

  this.log = function (msg) {
    logHandler.info('Cache: ' + msg);
  };

  this.debug = function (msg) {
    logHandler.log('Cache: ' + msg);
  };
  ...
  this.redisSNIClient = redis.createClient(
    this.config.redisPort,
    this.config.redisHost
  );
  ...
  this.redisSNIClient.on('error', function (err) {
    this.log('redisSNIClientError ' + err);
  }.bind(this));
...
Cache.prototype.watchSNIConfig = function (callback) {
  var channelName = "sni configuration";
  var self = this;

  this.redisSNIClient.on("message", function(channel, message) {
    if (channel == channelName && message == "reload") {
      self.getSNIConfig(callback);
    }
  });
  this.redisSNIClient.subscribe(channelName);
};

Cache.prototype.getSNIConfig = function (callback) {
  var multi = this.redisClient.multi();

  this.redisClient.keys('sni:*', function(err, values) {
