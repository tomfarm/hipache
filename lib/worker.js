...
// TODO: always add x-debug-headers and strip on the end if debug ==
// false
// this is nicer than the if all over the place

// function stripHeaders(headers) {}
//
//
// TODO: extract Backend status (dead or not)
//       extract static pages http server
//       extract proxy
//       extract redirector
//       add redirect to root path
//       extract logging
//       add per vhost ssl
//
...
//Backend.prototype = new events.EventEmitter();


Worker.prototype.runServer = function (config) {

  var backendIsDead = function(err, req) {
    var status = err.code === 'ECONNREFUSED' ||
                 err.code === 'ETIMEDOUT' ||
                 req.error !== undefined
  ...
  var logBackendError = function(err, req) {
     var backendId = req.meta.backendId;

      log(req.headers.host + ': backend #' + req.meta.backendId +
          ' reported an error (' + JSON.stringify(err) +
          ') while handling request for ' + req.url);
  }

  var logDeadBackend = function(err, req) {
     var backendId = req.meta.backendId;

     log( req.headers.host + ': backend #' + backendId + ' is dead (' +
          JSON.stringify(err) + ') while handling request for ' + req.url);
  }

  var clearDeadBackend = function(req) {
  ...
  var proxyErrorHandler = function (err, req, res) {
    ...
    if (backendIsDead(err, req)) {
      logDeadBackend(err, req, true)
      clearDeadBackend(req)
    } else {
      logBackendError(err, req, false)
    }
    ...
    // TODO: what happens if config.retryOnError is not set?
    if (req.retries >= config.retryOnError) {
      if (config.retryOnError) {
        log(req.headers.host + ': Retry limit reached ('
            + config.retryOnError + '), aborting.');
  ...
  var startHandler = function (req, res) {
    ...
    _.each(default_headers, function(value, header) {
  ...
  var getRemoteAddress = function (req) {
    ...
    var fillIpv6Address = function(remoteAddr) {
    ...
    if (req.connection.remoteAddress) {
      ...
      address = fillIpv6Address(address)
    }
